...
# Load raw count data generated by HTseq
rawCounts <- read.table("~/metaReadCount_refSeq.xls",row.name=1, header=TRUE, fill=TRUE)
...
conds<-read.csv("~/target.csv",head=T,stringsAsFactors=FALSE,row.name=1)
rownames(conds)<-conds$sample
#colnames(rawCounts)<-gsub("\\.","_",colnames(rawCounts))
conds<-conds[order(conds$group),];conds
rawCounts<-rawCounts[,conds$sample]
conds

# Some filtering for low count genes (e.g. retain genes with min 5 count for at least 3 samples)
temp<-rawCounts
temp$flag<-0
for (i in 1:ncol(rawCounts)){
  temp$flag<-temp$flag+(temp[,i]>=5)
}
table(temp$flag>=3) #22208 clusters
rawCounts<-rawCounts[temp$flag>=3,]

conds
#############################################
# Load annotation data
genes <- read.table("~/mouse.annotation.txt", header=TRUE, row.names=1)
...
clab
##################################
# Initialize data for limma voom #
##################################
x<-rawCounts
...
##################################################################
my.contrasts<-makeContrasts("Dpi1w_vs_Intact"=Dpi1w - Intact,
                            "Dpi3w_vs_Intact"=Dpi3w - Intact,
                            "Dpi3w_vs_Dpi1w"=Dpi3w - Dpi1w,
                            levels=design)
my.contrasts
...
decide <- matrix(c("fdr",0.05, "fdr",0.1,"fdr",0.2,"none",0.001,"none",0.005, "none", 0.01),nrow=6,ncol=2,byr=T)
...
# Generate FPKM data #
######################
# Run the following script only first time
#mm10gtf<-read.table("/Users/rikikawaguchi/Dropbox (UCLA Mednet)/RNAseq-PIPELINE/mm10.gtf",sep="\t")
...
#write.csv(geneLen,"~/RNAseq-PIPELINE/mm10_refSeq_geneLen.csv")

geneLen<-read.csv("~/RNAseq-PIPELINE/mm10_refSeq_geneLen.csv",head=T,row.names=1,stringsAsFactors = F);head(geneLen)
...
heatmap.plus(matcor2, col=rev(heat.colors(100)), main="Sample Pearson correlation - Normalized", ColSideColors=clab, cexRow=1, cexCol=1, margins=c(17,20))
...
heatmap.plus(matcor2, Colv=NA,col=rev(heat.colors(100)), main="Sample Pearson correlation - Normalized", ColSideColors=clab, cexRow=1, cexCol=1, margins=c(17,20))
legend("topright",legend=unique(conds$group),ncol=as.integer(length(unique(conds$group))/10)+1,fill=unique(clab[,2]),cex=1)
dev.off()
...
heatmap.plus(voom.data$E[topDEG, ], col=bluered(100), ColSideColors=clab, main=paste(" Top 100 DEG at FDR <",padj,sep=""), cexCol=0.9, cexRow=0.7,margins=c(10,15))
...
heatmap.plus(as.matrix(DESeq.vst.count[topDEG, ]), col=bluered(100), ColSideColors=clab, main=paste(" Top 100 DEG at FDR <",padj,sep=""), cexCol=0.9, cexRow=0.7,margins=c(10,15))
legend("topright", legend=unique(conds$group), fill=unique(clab[,2]),cex=0.7)
dev.off()
...
heatmap.plus(log2(w$pseudo.counts[topDEG, ]+0.0001), col=bluered(100), ColSideColors=clab, main=paste(" Top 100 DEG at FDR <",padj,sep=""), cexCol=0.9, cexRow=0.7,margins=c(10,15))
legend("topright", legend=unique(conds$group), fill=unique(clab[,2]),cex=0.7)
dev.off()
...
temp<-cbind(cpComp1[,c(grep("logFC",colnames(cpComp1)),grep("FDR",colnames(cpComp1)))],cpComp2[,c(grep("logFC",colnames(cpComp2)),grep("FDR",colnames(cpComp2)))],cpComp3[rownames(cpComp3),c(grep("logFC",colnames(cpComp3)),grep("FDR_",colnames(cpComp3)))])
...
#dim(cpComp1)
#temp6<-cpComp1[,c(1,grep("^pVal_",colnames(cpComp1)),grep("logFC_",colnames(cpComp1)))]
#library(plyr)
#avg<-function(x){
#  return(mean(x[,2]))
#}
...
#  fname<-gsub("pVal_","Signed_log10pVal_",colnames(temp6)[i],)
...
#######################
#dim(cpComp2)
#temp6<-cpComp2[,c(1,grep("^pVal_",colnames(cpComp2)),grep("logFC_",colnames(cpComp2)))]
...
#cpComp3<-read.csv("~/Dropbox (UCLA Mednet)/Projects_2016+/Projects_2019/2019-9214_Yuqin/3a__DEA_edgeR/Complete_geneList.csv",head=T,row.names=NULL,stringsAsFactors = F)
cpComp3<-cpComp3[!duplicated(cpComp3$gene),]
rownames(cpComp3)<-cpComp3$gene
temp6<-cpComp3[,c(1,grep("PValue_",colnames(cpComp3)),grep("logFC_",colnames(cpComp3)))]
library(plyr)
avg<-function(x){
  return(mean(x[,2]))
}
...
for (i in 2:dim(temp7)[2]){
  ...
  dim(temp8)
  temp8$gene<-toupper(temp8$gene)
  fname<-gsub("PValue_","Signed_log10pVal_",colnames(temp6)[i],)
  temp8<-temp8[complete.cases(temp8),]
